...
# ------------------------------------------------------------------------
# The trained .keras model and the MFCC standardization stats .npz live in
# a Hugging Face Hub MODEL repo. The original precomputed CWT/MFCC dataset
# folders live in a SEPARATE Hugging Face Hub DATASET repo (that's how the
# team actually uploaded them - two different repos, two different repo
# types). GitHub rejects files over 100 MB, and Google Drive links hit
# virus-scan interstitials / download quotas under repeated programmatic
# access - Hugging Face Hub is built for exactly this: serving model +
# dataset files to an app at scale.
#
# On first run, `model_utils.py` / `dataset_lookup.py` download what they
# need from these repos into local folders and cache them there for every
# following run (st.cache_resource -> at most once per server process).
#
# Everything below is already filled in with Ayman's team's repos:
#   https://huggingface.co/noonatu/cad-heart-sound            (model repo)
#   https://huggingface.co/datasets/noonatu/cad-heart-sound-dataset (dataset repo)
# so the app works out of the box. If you fork this and use your own HF
# repos, change the values below (or override via Streamlit secrets / env
# vars of the same name - secrets always win over the hardcoded values).
...
HF_DATASET_REPO_ID = os.environ.get("HF_DATASET_REPO_ID", "noonatu/cad-heart-sound-dataset")
HF_DATASET_REPO_TYPE = os.environ.get("HF_DATASET_REPO_TYPE", "dataset")

# Only needed if either HF repo is/becomes private. Public repos work fine
# with this left empty. Set as a Streamlit secret named HF_TOKEN, never
# hardcode a real token here if these repos are public.
...
# are part of the original dataset. They're pulled from HF_DATASET_REPO_ID
# above. Unlike the old Google Drive setup, Hugging Face Hub handles many
# small files and repeated downloads without quota/virus-scan issues, so
# this is ON by default now. Set HF_SYNC_DATASET="false" (as a secret or
# env var) to disable it and always fall back to on-the-fly extraction.
...
try:  # pull from Streamlit secrets when running as a deployed app
    ...
    HF_DATASET_REPO_ID = _st.secrets.get("HF_DATASET_REPO_ID", HF_DATASET_REPO_ID)
    HF_DATASET_REPO_TYPE = _st.secrets.get("HF_DATASET_REPO_TYPE", HF_DATASET_REPO_TYPE)
