...
from config import (
    ...
    HF_REPO_ID, HF_REPO_TYPE, HF_TOKEN, HF_SYNC_DATASET,
)
...
def _sync_dataset_from_hf():
    ...
    the Hugging Face Hub repo (see HF_SYNC_DATASET in config.py). Runs at
    most once per server process thanks to st.cache_resource. Silently does
    nothing (and the app keeps working via on-the-fly extraction) if the
    flag is off, `huggingface_hub` isn't installed, or the download fails."""
