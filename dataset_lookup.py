...
from config import (
    CLASS_NAMES, CWT_DIR, MFCC_DIR, N_MFCC,
    GDRIVE_CWT_FOLDER_ID, GDRIVE_MFCC_FOLDER_ID, GDRIVE_SYNC_DATASET,
)


@st.cache_resource(show_spinner=False)
def _sync_dataset_from_drive():
    """One-time, opt-in download of the precomputed CWT/MFCC dataset folders
    from Google Drive (see GDRIVE_SYNC_DATASET in config.py). Runs at most
    once per server process thanks to st.cache_resource. Silently does
    nothing (and the app keeps working via on-the-fly extraction) if the
    flag is off, `gdown` isn't installed, or the download fails."""
    if not GDRIVE_SYNC_DATASET:
        return False
    try:
        import gdown
    except ImportError:
    ...
    ok = True
    if GDRIVE_CWT_FOLDER_ID and not os.path.isdir(CWT_DIR):
        try:
            os.makedirs(os.path.dirname(CWT_DIR), exist_ok=True)
            gdown.download_folder(id=GDRIVE_CWT_FOLDER_ID, output=CWT_DIR, quiet=False, use_cookies=False)
        except Exception:
            ok = False
    if GDRIVE_MFCC_FOLDER_ID and not os.path.isdir(MFCC_DIR):
        try:
            os.makedirs(os.path.dirname(MFCC_DIR), exist_ok=True)
            gdown.download_folder(id=GDRIVE_MFCC_FOLDER_ID, output=MFCC_DIR, quiet=False, use_cookies=False)
        except Exception:
            ok = False
    return ok
...
def find_match(filename: str):
    ...
    """
    if GDRIVE_SYNC_DATASET and not (os.path.isdir(CWT_DIR) and os.path.isdir(MFCC_DIR)):
        with st.spinner("Syncing precomputed CWT/MFCC dataset from Google Drive (first run only)..."):
            _sync_dataset_from_drive()
